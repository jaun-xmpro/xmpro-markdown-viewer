...
${body}</div>\n`;
    ...
    // limit concurrent contexts per document, so reusing one keeps repeated
    // hold attempts safe.
    let audioCtx = null;
    ...
    function enterStarwars() {
        ...
        // contents are a real DOM copy without serialising via innerHTML.
        // Strip any live search highlights so they don't bleed into the crawl.
        const md = els.content.querySelector('.md');
        const cloned = md ? md.cloneNode(true) : els.content.cloneNode(true);
        cloned.querySelectorAll('.search-highlight').forEach(el => {
            el.replaceWith(document.createTextNode(el.textContent));
        });
        els.starwarsCrawl.replaceChildren(cloned);
        els.starwarsOverlay.classList.remove('playing');
        els.starwarsOverlay.hidden = false;
        // void offsetWidth forces a layout flush so the play-state restart
        // takes effect — without it, removing+re-adding .playing in one
    ...
    function getReadableText(startFrom) {
        ...
        // If startFrom is inside an excluded subtree (e.g. user clicked
        // inside a code block), advance past the whole block to the next
        // element. Without this, the walker would reject startFrom itself
        // and silently fall back to reading from the document start.
        if (startFrom) {
            let outermostExcluded = null;
            for (let el = startFrom; el && el !== content; el = el.parentElement) {
                if (excluded.has(el)) outermostExcluded = el;
            }
            if (outermostExcluded) startFrom = outermostExcluded.nextElementSibling || null;
        }
        ...
        while ((n = walker.nextNode())) {
            if (!started) {
                ...
                if (!(pos === 0 || (pos & Node.DOCUMENT_POSITION_FOLLOWING))) continue;
                started = true;
            }
